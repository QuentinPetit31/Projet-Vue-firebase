...
function deleteFriend(id) {
  const index = lesAmis.findIndex((a) => a.id === id);
  if (index !== -1) {
    const deletedFriend = lesAmis.splice(index, 1);
    console.log(`Ami ${deletedFriend[0].name} supprimé.`);
    alert(`Ami ${deletedFriend[0].name} a été supprimé.`);
  } else {
    console.error(`Erreur : Aucun ami trouvé avec l'ID ${id}`);
  }
}
